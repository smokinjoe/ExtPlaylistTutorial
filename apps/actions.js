...
const youtubeApiKey = 'AIzaSyCH-FQ6lodHV13VKLDEgsMdGkuZzctBdXI';
const youtubeApiBaseUrl = 'https://googleapis.com/youtube/v3';
...
const _searchVideos = (dispatch, keyword, nextPageToken = null) => {
  ...
  return fetch(url)
    .then((resp) => respon.json())
    .then((data) => {
      if (data.error) {
        throw data.error.message || 'Unable to search';
      }
